...
export const CardColumns = ({ blok, settings }: any) => {
  return (
    <div className="container-section mx-auto">
      ...
      transition-all duration-300 ${item.link.cached_url && "hover:shadow-lg"} `}
            >
              {item.image.filename && (
                <div className="w-full h-[300px] relative">
                  <Image
                    ...
                    fill
                    alt={item.title}
                    className={`${settings.content.border_template && "rounded-lg"} object-cover`}
                  />
                </div>
              )}
              <div className="flex flex-col gap-2">
                ...
                <span
                  className={`flex flex-col gap-5 ${blok.columns === "1" && "lg:max-w-[80%]"}`}
                >
                  {/* change to: */}
                  {render(item.content)}
                </span>
      ...
      lg:p-14 transition-all duration-300 ${item.link.cached_url && "hover:shadow-lg"} `}
            >
              {item.image.filename && (
                <div className="w-full h-[300px] relative">
                  <Image
